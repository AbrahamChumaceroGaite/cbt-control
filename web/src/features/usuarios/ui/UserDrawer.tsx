...
export function UserDrawer({ user, onClose, onUpdated }: Props) {
  ...
  const gradient  = isAdmin
    ? 'from-purple-900/60 via-purple-800/20 to-transparent'
    : 'from-blue-900/60 via-blue-800/20 to-transparent'
  ...
  const sections  = (user?.student
    ? ['profile', 'notifications', 'transactions']
    : ['profile', 'notifications']) as DrawerSection[]
  ...
  return (
    <>
      ...
      {/* Panel — full width on mobile, max-w-md on sm+ */}
      <div
        className={`fixed right-0 top-0 bottom-0 w-full sm:max-w-md bg-zinc-950 border-l border-zinc-800 shadow-2xl flex flex-col transition-transform duration-300 ${user ? 'translate-x-0' : 'translate-x-full'}`}
        style={{ zIndex: Z.DRAWER + 1 }}
      >
        {!user ? null : (
          <>
            {/* ── Profile cover ─────────────────────────────────── */}
            <div className={`relative flex-shrink-0 bg-gradient-to-b ${gradient} border-b border-zinc-800`}>
              {/* Decorative circle */}
              <div className="absolute right-4 top-1/2 -translate-y-1/2 w-32 h-32 rounded-full opacity-[0.07] pointer-events-none"
                style={{ background: isAdmin ? 'radial-gradient(circle, #a855f7, transparent)' : 'radial-gradient(circle, #3b82f6, transparent)' }} />

              {/* Close button */}
              <button
                onClick={onClose}
                className="absolute top-3 right-3 p-1.5 rounded-lg text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800/60 transition-colors z-10"
                aria-label="Close"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="flex items-center gap-4 p-5 pt-6">
                {/* Avatar */}
                <div className={`w-20 h-20 rounded-2xl border-2 flex items-center justify-center text-2xl font-black flex-shrink-0 shadow-lg ${avatarCls}`}>
                  {user.initial}
                </div>

                {/* Info */}
                <div className="min-w-0 flex-1 z-10">
                  <h3 className="font-bold text-zinc-100 text-lg truncate leading-tight">{user.displayName}</h3>
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${roleCls}`}>
                      {isAdmin ? 'Admin' : 'Student'}
                    </span>
                    <span className={`text-xs font-medium flex items-center gap-1 ${user.isActive ? 'text-emerald-400' : 'text-zinc-500'}`}>
                      <span className={`inline-block w-1.5 h-1.5 rounded-full ${user.isActive ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                      {user.isActive ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  {user.code && (
                    <p className="text-[10px] text-zinc-600 mt-1 font-mono">{user.code}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="p-4 border-t border-zinc-800 flex-shrink-0 flex flex-col gap-2">
              <div className="flex gap-2">
                <Button size="sm" onClick={h.openEdit} className="flex-1">
                  <UserCog className="w-3.5 h-3.5 mr-1.5" /> Edit
                </Button>
