...
import { Plus, Bell, BellOff, SlidersHorizontal, X, ChevronDown } from 'lucide-react'
...
export function UsuariosSection({ showToast }: Props) {
  ...
  // Unique course names from users
  ...
  const filtered = users.filter(u => {
    ...
    if (filters.from) {
      ...
      if (created < new Date(filters.from).getTime()) return false
    }
    if (filters.to) {
      const created = new Date(u.createdAt).getTime()
    ...
    <>
      <div className="space-y-4 animate-in fade-in duration-300">
        ...
        <div className="flex items-start gap-2">
          ...
          {/* Filter button */}
          <div className="relative mt-6" ref={filterRef}>
            <button
            ...
            >
              ...
              Filtros
              {active && <span className="w-1.5 h-1.5 rounded-full bg-amber-400 ml-0.5" />}
              <ChevronDown className={`w-3 h-3 transition-transform ${showFilters ? 'rotate-180' : ''}`} />
            </button>

            {/* Filter panel */}
            {showFilters && (
              <div className="absolute right-0 top-[calc(100%+6px)] z-[200] w-72 bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl shadow-black/60 p-4 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-zinc-300">Filtros</span>
                  {active && (
                    <button onClick={() => { setFilters(DEFAULT_FILTERS); setPage(0) }} className="text-[10px] text-zinc-500 hover:text-amber-400 transition-colors flex items-center gap-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-zinc-500 uppercase tracking-wide">Rol</label>
                    <select
                      value={filters.role}
                      onChange={e => { setFilters(p => ({ ...p, role: e.target.value as RoleFilter })); setPage(0) }}
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={e => { setFilters(p => ({ ...p, status: e.target.value as StatusFilter })); setPage(0) }}
                      className="w-full bg-zinc-900 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-zinc-500"
                    >
                      <option value="all">Todos</option>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] text-zinc-500 uppercase tracking-wide">Push</label>
                    <select
                      value={filters.push}
                      onChange={e => { setFilters(p => ({ ...p, push: e.target.value as PushFilter })); setPage(0) }}
                      className="w-full bg-zinc-900 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-zinc-500"
                    >
                      <option value="all">Todos</option>
                      <option value="active">Con push</option>
                      <option value="inactive">Sin push</option>
                    </select>
                  </div>
