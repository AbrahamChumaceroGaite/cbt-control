        ...
        {Array.from({ length: status.limit }).map((_, i) => (
          <div
            ...
            className={`flex-1 h-2 rounded-full transition-all duration-500 ${
              i < status.used ? 'bg-amber-500' : 'bg-zinc-800'
            }`}
          />
        ))}
      </div>
...
// ─── Recipient picker (course → student) ─────────────────────────────────────

interface RecipientPickerProps {
  onSelect: (s: StudentSearchResult) => void
}

function RecipientPicker({ onSelect }: RecipientPickerProps) {
  ...
  useEffect(() => {
    portalService.getCourses().then(setCourses).catch(() => {})
  }, [])

  // Search when q or courseId changes (only if course selected + q >= 2 chars)
  useEffect(() => {
    ...
    <div className="space-y-2.5">
      ...
      <div className="relative">
        <select
          ...
          className="w-full h-10 pl-3 pr-8 rounded-xl bg-zinc-800/80 border border-zinc-700 text-sm text-zinc-200 focus:outline-none focus:border-amber-500/50 transition-colors appearance-none"
        >
      ...
      {/* Name search — only enabled after course selected */}
      <div className="relative">
        ...
        <input
          ...
          disabled={!courseId}
          placeholder={courseId ? 'Buscar alumno por nombre…' : 'Primero selecciona un curso'}
...
function SendForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Step 1 — Recipient */}
      <div>
        <label className="block text-[10px] font-extrabold uppercase tracking-[0.15em] text-zinc-500 mb-2">
          Destinatario
        </label>
        {recipient ? (
      ...
      {/* Steps 2+ — only visible after recipient selected */}
      {recipient && (
        <>
          {/* Amount */}
          <div>
          ...
          <button
            ...
            className={`w-full h-11 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98] ${
              canSend
                ? 'bg-amber-500 hover:bg-amber-400 text-zinc-900 shadow-[0_0_20px_rgba(245,158,11,0.25)]'
