    ...
    <div className="space-y-2">
      <div className="flex gap-2">
        <div className="flex-shrink-0 w-36">
          <Select value={courseId} onChange={e => onCourseChange(e.target.value)}
            className="h-10 rounded-xl text-xs text-zinc-200">
        ...
        <div className="relative flex-1">
          ...
          <input type="text" value={q} onChange={e => onQChange(e.target.value)} disabled={!courseId}
      ...
      {results.length > 0 && (
        <div className="rounded-xl border border-zinc-700/60 bg-zinc-900 overflow-hidden shadow-2xl">
          {results.map(s => (
            <button key={s.id} onClick={() => { onSelect(s); onClearSearch() }}
