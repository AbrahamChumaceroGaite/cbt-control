...
interface Props { logs: StudentData['coinLogs']; currentCoins: number }

export function TrajectoryChart({ logs, currentCoins }: Props) {
  const ordered = [...logs].reverse()
  if (ordered.length < 3) return <p className="text-xs text-zinc-600 text-center py-4">Sin suficientes datos</p>
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={120}>
        <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="coins" stroke={strokeColor} strokeWidth={2} fill="url(#coinGrad)"
            dot={false} activeDot={{ r: 4, fill: '#fbbf24', stroke: '#1c1c1c', strokeWidth: 2 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
