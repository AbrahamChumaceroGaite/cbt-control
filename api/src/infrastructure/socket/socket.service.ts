...
/**
 * SocketService — real-time event bus using Server-Sent Events (SSE).
 *
 * Replaces socket.io with plain HTTP streaming so no WebSocket upgrade or
 * special proxy configuration is needed. Each connected browser tab gets its
 * own Subject; events are routed by role (admin / student).
 */
@Injectable()
export class SocketService {
  ...
  // ── Subscription (called by SseController) ───────────────────────────────
  ...
  // ── Public API (same shape as before — callers need no changes) ───────────
